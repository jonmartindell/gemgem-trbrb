...
class MemoCreateTest < MiniTest::Spec
  ...
  it "rendering" do # DISCUSS: not sure if that will stay here, but i like the idea of presentation/logic in one place.
    form = Memo::Create.present({}).contract

    form.date.must_equal nil
    form.completed_tasks.must_equal nil
    form.future_plans.must_equal nil
    form.notes_to_self.must_equal nil
    form.successes.must_equal nil
    form.disappointments.must_equal nil
    form.expenses.must_equal nil
  end

  it "persists valid" do
    memo = Memo::Create[memo: {date: "2016-01-01", completed_tasks: "planted seeds"}].model

    memo.persisted?.must_equal true
    memo.date.must_equal Time.new(2016,01,01).to_date
    memo.completed_tasks.must_equal "planted seeds"
  end

  it "invalid" do
    res, op = Memo::Create.run(memo: {})

    op.errors.to_s.must_equal "{:base=>[\"something must be filled in\"]}"
